...
if __name__ == '__main__':
    ...
    mytempfile='~/.vdcapi.storagetestuser' 
    ...
    ### We wil create a network specifically for the test ###
    network_name='validation-storage-%s-%d-net' %(user_name, time.time())
    displaytext=network_name
    ### First we create the networks

    ### network_id=create_network(zone_id, domain_id, network_name, api)
    ...
    process = multiprocessing.Process(target=storage_test, args=(
        zone_id,
        network_name,
        template_id,
        domain_id,
    ...
    process.name = 'storage_test@%s' % network_name
    process.start()
